...
class AuthState {
  ...
  AuthState copyWith({
    ...
    return AuthState(
      ...
      error: error,
      samplePrompts:
          samplePrompts ?? this.samplePrompts, // ✅ NEW: Include in copyWith
    );
  }
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      state = state.copyWith(
        ...
        samplePrompts: _authService
            .samplePrompts, // ✅ NEW: Get sample prompts from service
      );
    ...
    try {
      ...
      if (userInfo != null) {
        state = state.copyWith(
          ...
          userInfo: userInfo,
          samplePrompts: _authService
              .samplePrompts, // ✅ NEW: Update sample prompts after sign-in
        );
        developer.log(
          '✅ Sign-in successful in state notifier',
          name: 'AuthStateNotifier',
        );
      } else {
        state = state.copyWith(isLoading: false, error: 'Sign-in failed');
        developer.log(
          '⚠ Sign-in failed in state notifier',
          name: 'AuthStateNotifier',
        );
      }
    } catch (e) {
      developer.log(
        '❌ Sign-in error in state notifier: $e',
        name: 'AuthStateNotifier',
      );
      state = state.copyWith(isLoading: false, error: 'Sign-in error: $e');
    }
  }

  Future<void> signOut() async {
    developer.log(
      '🔴 Starting sign-out in state notifier',
      name: 'AuthStateNotifier',
    );
    try {
      ...
      developer.log(
        '✅ Sign-out completed in state notifier',
        name: 'AuthStateNotifier',
      );
    } catch (e) {
      developer.log(
        '❌ Sign-out error in state notifier: $e',
        name: 'AuthStateNotifier',
      );
